...
W, H = 1800, 2480
INK, MUTED, GREEN = '#163846', '#506b79', '#11796d'
parts = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" role="img" aria-labelledby="title desc">',
 ...
 '<defs><marker id="arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0 L10 5 L0 10Z" fill="#258f7e"/></marker><marker id="light" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0 L10 5 L0 10Z" fill="#7ce3c7"/></marker></defs>',
 '<style>text{font-family:"Microsoft YaHei","Noto Sans CJK SC","Segoe UI",sans-serif}a:hover text{text-decoration:underline}</style>',
 f'<rect width="{W}" height="{H}" fill="#f3f7f9"/>']

def rect(x,y,w,h,fill='#fff',stroke='#d6e3e7',r=15):
...
for i,(title,tag,body) in enumerate(cases):
    x=48+i*344
    rect(x,248,328,200)
    txt(x+20,283,tag,19,GREEN if i<2 else '#927041',600,288)
...
for i,(title,body,foot) in enumerate(inputs):
    ...
    txt(x+22,708,foot,20,GREEN,600,514)

section(788,'03','内部怎样处理？','把“背景关系”转换为“角色行动”，再由行动结果进入下一轮')
rect(48,816,1704,750,'#12323f','#12323f',20)
txt(76,852,'准备阶段  /  从材料建立模拟起点',21,'#88ddc8',600)
pre=[
...
for i,(title,body) in enumerate(pre):
    x=76+i*554
    rect(x,876,518,143,'#1c4351','#40616d',12)
    txt(x+20,914,title,26,'#f2f9f9',700,478)
    lines(x+20,956,body,22,'#c6dce2',gap=33,maxw=478)
    if i<2:arrow(x+525,948,x+545,948,True)
parts.append('<path d="M1445 1022 V1041 H614 V1058" fill="none" stroke="#7ce3c7" stroke-width="2.5" marker-end="url(#light)"/>')
rect(76,1070,1074,327,'#173c49','#446671',12)
txt(98,1106,'多轮互动循环  /  活跃角色按当前可见信息行动',25,'#f2f9f9',700)
loop=[('观察','角色背景与记忆','当前可见帖子 / 环境信息'),('决策 · 大模型','人设 + 上下文 → 选择行动','同一模型可驱动多个角色'),('执行 · OASIS','发帖、评论、转发等','也可以不行动')]
for i,(title,a,b) in enumerate(loop):
    x=98+i*346
    rect(x,1133,311,131,'#eaf6f1','#eaf6f1',10)
    txt(x+17,1170,title,26,GREEN,700,277)
    lines(x+17,1209,[a,b],20,INK,gap=30,maxw=277)
    if i<2:arrow(x+317,1198,x+337,1198,True)
txt(115,1338,'共享环境保存行动结果；可用信息与记忆更新 → 其他角色在后续轮次观察',23,'#d4e9ed',maxw=1000)
parts.append('<path d="M1049 1271 V1300 H252 V1274" fill="none" stroke="#7ce3c7" stroke-width="2.5" marker-end="url(#light)"/>')
txt(416,1375,'不是每人都发言，也不是所有角色都知道一切',21,'#88ddc8',600,690)

rect(1174,1070,550,468,'#1c4351','#40616d',12)
txt(1196,1110,'几个容易混淆的分工',26,'#f2f9f9',700,506)
lines(1196,1153,['角色 = 档案 / 人设 + 上下文 + 可用动作','大模型：生成设定、选择行动、分析结果','OASIS：执行动作，管理虚拟社交环境','Zep：组织知识关系与记忆检索','MiroFish：编排流程，提供网页与接口'],22,'#c6dce2',gap=38,maxw=506)
rect(1196,1340,506,171,'#143744','#527080',10)
lines(1212,1373,['多个 Agent ≠ 必须使用多种模型','角色定制 ≠ 为每个角色训练模型','社交模拟 ≠ 完整的现实 / 剧情引擎','报告通常负责事后分析，不是剧情导演'],21,'#8fe1cc',gap=36,maxw=474)

arrow(613,1400,613,1417,True)
rect(76,1426,1074,112,'#eaf6f1','#eaf6f1',12)
txt(98,1465,'模拟后的分析 · ReportAgent',28,GREEN,700)
...
for i,(title,form,body) in enumerate(outputs):
    ...
    txt(x+262,y+37,form,20,GREEN,600,274)
    lines(x+22,y+85,body,23,gap=35,maxw=514)

section(2056,'05','怎样理解它的价值？','用模拟形成可检查的假设，再回到现实验证')
rect(48,2085,832,215,'#e5f4ed','#b9dbcd')
txt(72,2127,'能帮助你做的事',28,GREEN,700)
lines(72,2171,['发现被遗漏的群体、顾虑和沟通缺口；探索可能的反应路径。','设计不同方案的模拟，形成访谈问题、评审线索与创作方向。','知识库侧重“资料说了什么”；它进一步探索“角色可能怎么做”。'],23,INK,gap=39,maxw=784)
rect(902,2085,850,215,'#fff4e8','#e4c9a8')
txt(926,2127,'不能由模拟直接推出的结论',28,'#89602f',700)
lines(926,2171,['模拟客户不等于真实市场样本，角色数量不等于代表性。','舆情推演不等于全网实时监测；故事推演不保证成稿小说。','反复出现的结果不自动等于现实概率，准确性需对照验证。'],23,'#6a5847',gap=39,maxw=802)
...
for i,(label,path) in enumerate(links):
    ...
    txt(48+i*420,2428,label+' ↗',21,GREEN,600,390)
    parts.append('</a>')
parts.append('</svg>')
