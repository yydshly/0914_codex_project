...
W, H = 1600, 1830
INK, MUTED, GREEN = '#153442', '#506874', '#127569'
parts = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" role="img" aria-labelledby="title desc">',
         ...
         '<defs><marker id="arrow" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto"><path d="M0 0 L8 4.5 L0 9" fill="none" stroke="#72968d" stroke-width="1.5"/></marker></defs>',
         '<style>text{font-family:"Microsoft YaHei","Noto Sans CJK SC","Segoe UI",sans-serif}</style>',
         f'<rect width="{W}" height="{H}" fill="#f3f7f9"/>']

def rect(x, y, w, h, fill='#fff', stroke='#d6e2e7', radius=12):
...
def arrow(x1,y1,x2,y2):
    parts.append(f'<path d="M{x1} {y1} L{x2} {y2}" fill="none" stroke="#72968d" stroke-width="2.3" marker-end="url(#arrow)"/>')

def step(y, h, number, title, sub):
    rect(45,y,990,h)
    rect(65,y+21,48,40,'#e8f5ef','#e8f5ef',8)
    text(75,y+50,number,23,GREEN,700)
    text(132,y+51,title,28,INK,700)
    text(132,y+83,sub,20,GREEN)

text(45,45,'MIROFISH / 多智能体社会推演',20,GREEN,700)
...
for i,(title,body) in enumerate(cases):
    x=45+i*382
    rect(x,179,364,100,'#e9f1f3','#d9e6ea')
    text(x+20,216,title,25,INK,700)
    text(x+20,253,body,21,MUTED)

rect(45,300,1510,138,'#153442','#153442')
text(69,336,'先搭运行环境',25,'#80e0c6',700)
lines(69,375,['MiroFish 网页 + Python 后端','提供操作入口，编排各个步骤'],22,'#e1eef2',32)
lines(535,375,['大模型 API + Zep 服务','模型负责生成与判断，Zep 管理图谱与记忆'],22,'#e1eef2',32)
lines(1150,375,['OASIS 仿真引擎','提供虚拟社交环境'],22,'#e1eef2',32)
...
for i,(title,body) in enumerate(roles):
    x=70+i*316
    rect(x,945,299,80,'#eff7f3','#d8eae2',8)
    text(x+14,976,title,24,GREEN,700)
...
rect(1080,865,455,43,'#e7f5ef','#e7f5ef',7)
text(1094,894,'这是配置角色，不是训练一个新模型。',21,GREEN,700)

rect(1060,949,495,354)
text(1083,991,'需要多个不同的 AI 吗？',26,INK,700)
text(1083,1031,'需要多个 Agent，不必用多种大模型。',22,GREEN,600)
rect(1179,1055,263,55,'#153442','#153442',8)
text(1201,1091,'同一个大模型 API',25,'#e6f5ef',700)
for x,title in [(1090,'角色 A'),(1247,'角色 B'),(1404,'角色 C')]:
    arrow(1310,1113,x+60,1140)
    rect(x,1148,126,49,'#edf7f2','#b9dcd0',7)
    text(x+21,1180,title,23,GREEN,600)
...
rect(45,1611,1510,124,'#e6f3ed','#bad9cb')
text(70,1654,'适合得到：可能的反应路径、分歧和遗漏问题，供方案评审与真实调研使用。',27,GREEN,700)
