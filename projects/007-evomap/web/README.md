...
源码是 `index.html`、`style.css`、`main.js`。构建脚本只复制这三个文件、两份明确列出的实验摘要，以及两张理解图各自的 PNG 与 SVG，共 9 个文件，排除源码缓存、环境配置和依赖。`dist/` 可重新构建，不进入版本管理。
...
2026-09-16 首版部署成功，首版线上 7 个静态文件均返回 HTTP 200，并核对与本地构建内容一致（文本统一换行符后比较）；同时验证总导航与 001–007 全部入口可访问。蜂群章节新增两份图像资源，当前构建白名单为 9 个文件。
