...
**蜂群可以理解成一支临时 AI 团队：有人分工、有人执行、有人检查；遇到问题可以返工或换人，做过的方法还能留给以后复用。**

以“做一个能正常登录的网站”为例：设计 AI 给出方案，编程 AI 实现，测试 AI 检查；按钮无效就交回修改。技能沉淀是其中一部分，还要组织交接、处理失败、控制开销，并保留流程与协作反馈。角色是本次职责，能力取决于模型、工具、环境和权限。

![一眼看懂蜂群：几个 AI 协作做网站，交接、检查、返工、改派、控制预算并保存经验；概念示例，尚未实测云端收益](assets/swarm-quickstart.png)

[高清 PNG](assets/swarm-quickstart.png) · [可编辑 SVG](assets/swarm-quickstart.svg) · [网页引导](https://yydshly.github.io/0914_codex_project/007-evomap/#swarm-quickstart)

这是期望工作方式的示例，不是运行截图。已部署的是研究导读；EvoMap 云端蜂群是否提高质量、节省成本，仍待实测。
...
技能沉淀只是部分价值：还包括可复用流程、协作记录与编排反馈。新增 [蜂群能力清单](notes/swarm-capabilities.md) 和 [网页能力说明](https://yydshly.github.io/0914_codex_project/007-evomap/#swarm-capabilities)，覆盖组队、通信、审查返工、候补恢复、运行控制等 10 类能力；它们主要是平台描述，尚未云端实测。
