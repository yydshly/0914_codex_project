...
本子项目保存原版中文交互演示、架构全景图、真实界面截图和完整研究结论，远端研究页已嵌入同一套中文工作台，可直接搜索、换主题、编辑卡片和体验工作区。
...
研究页首屏提供“直接体验我的数字工作台”入口，并内嵌可操作界面，无需接入码。线上编辑保存到当前浏览器，不能写公共服务器配置；内存为固定示例 JSON，状态面板为明确标注的浏览器模拟，时钟实时运行。本机地址需要先启动下面的服务，才能体验真实本机信息和 HTTP 探测。
...
在线界面沿用本地演示的上游版本、中文卡片与六个分组，并非另外制作的仿制首页。`web/workbench/conf.yml` 是不含本机地址的公开配置。线上可以搜索、换主题、修改卡片并保存到浏览器、使用弹窗与 iframe 工作区。

构建先运行 `python src/build_workbench.py`，再运行 `python src/build_site.py`（在本项目目录）。首次构建需要 Node 24、Yarn 1.22.22 和联网；GitHub Actions 已配置完整构建流程。

- 保留固定上游版本与 MIT 许可证，构建输出不提交仓库。
- 使用子目录资源地址与 hash 路由，刷新工作区也能打开。
- 浏览器偏好使用 `dashy006:` 命名空间，服务端写盘被禁用。
- 在线构建关闭 Service Worker 注册与清理，不影响同域其他演示。
- `sample-data.json` 是固定示例；`control.html` 只在浏览器切换模拟状态，不冒充真实后端探测。
